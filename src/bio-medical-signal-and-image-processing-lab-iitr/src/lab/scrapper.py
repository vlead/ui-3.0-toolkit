...
def getIndexDotHtmlFiles():
	return filter(lambda fname: True if re.match(r'content.html*', fname) else False, \
	              os.listdir('.'))
...
def contentScrapper(fname):	
	...
	sectionContent = experimentContent.find_all('div', class_='divContent')
	if len(sectionContent) == 0:
		sectionContent = experimentContent.find_all('div', class_='divQuiz')
